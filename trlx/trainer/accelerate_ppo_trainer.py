...
from trlx.utils.modeling import RunningMoments, gather_dict, logprobs_of_labels, \
    calc_stepwise_kl, disable_dropout
...
class AcceleratePPOTrainer(AccelerateRLTrainer):
    ...
    def __init__(self, config: TRLConfig, kl_fn=None, kl_forward_fn=None, **kwargs):
        ...
        self.kl_fn = kl_fn
        self.kl_forward_fn = kl_forward_fn
        ...
        rollout_loader: DataLoader = self.store.create_loader(self.config.train.batch_size,
                                                              shuffle=True)
        ...
        if config.method.target is not None:
            self.kl_ctl = AdaptiveKLController(config.method.init_kl_coef,
                                               config.method.target, config.method.horizon)
        else:
        ...
        self.temperature = config.method.gen_kwargs["temperature"]

        if config.method.gen_experience_kwargs is not None:
            self.generate_experience_kwargs = {**generate_kwargs,
    ...
    def loss(self, batch: PPORLBatch) -> Tuple[float, Dict[str, Any]]:
        ...
        advantages, returns = self.config.method.get_advantages_and_returns(old_values,
                                                                            old_rewards,
                                                                            response_length)
        ...
        if self.config.model.model_arch_type == "seq2seq":
            ...
            attention_mask = input_ids.ne(self.tokenizer.pad_token_id).long().to(
                self.accelerator.device)
            decoder_attention_mask = (
                decoder_input_ids.ne(self.tokenizer.pad_token_id).long().to(
                    self.accelerator.device)
            )
            ...
            mask = decoder_input_ids.ne(self.tokenizer.pad_token_id).long().to(
                self.accelerator.device)
            start = 0
            ...
            attention_mask = tokens.not_equal(self.tokenizer.pad_token_id).long().to(
                tokens.device)
    ...
    def prepare_learning(self):
        ...
        self.total_steps = self.config.train.epochs * self.n_inner_epochs * len(
    ...
    def add_prompt_pipeline(self, pipeline: PromptPipeline):
        ...
        prompt_dataloader = pipeline.create_loader(self.config.method.chunk_size,
                                                   shuffle=True)
    ...
    def make_experience(self, num_rollouts: int = 1024, iter_count: int = 0):  # noqa:
        ...
        while len(ppo_rl_elements) < num_rollouts:
            ...
            prompt_sizes = torch.tensor([prompt_tensors.shape[1]] * len(prompt_tensors),
                                        device=device)
            ...
            metadata = gather_dict({k: v for k, v in batch.items() if
                                    k != "input_ids" and k != "attention_mask"})
            ...
            if self.accelerator.is_main_process:
                ...
                all_scores = list(
                    all_scores.reshape(self.accelerator.num_processes, -1, max_len).unbind())
            else:
                all_scores = None
            ...
            if self.config.method.cliprange_reward:
                scores = torch.clip(scores, -self.config.method.cliprange_reward,
            ...
            if self.config.model.model_arch_type == "seq2seq":
                ...
                # logging2.warning(f"all_tokens in ppo loop: {all_tokens[0]}")
                attention_mask = all_tokens.not_equal(self.tokenizer.pad_token_id).long().to(
                    device)
                position_ids = attention_mask.long().cumsum(-1) - 1
                position_ids.masked_fill_(attention_mask == 0, 1)
                with torch.no_grad():
                    # logging2.warning(f"p all_tokens {all_tokens[0, :]}")
                    ...
                    if hasattr(self.model, "frozen_head") or self.model.peft_type:
                        # logging2.warning(f"q all_tokens {all_tokens[0, :]}")
                        logging2.warning("HYDRA")
                        ref_logits = self.model.forward_hydra(
                            all_tokens,
                            attention_mask=attention_mask,
                            position_ids=position_ids,
            ...
            if self.config.model.model_arch_type == "seq2seq":
                ...
                step_wise_kl = calc_stepwise_kl(logits[:, :-1, :], ref_logits[:, :-1, :],
                                                temperature=self.temperature)
            ...
            step_kl_ratio = step_wise_kl * attention_mask[:, :-1]
            ...
            step_kl_penalty = self.kl_ctl.value * -step_kl_ratio.cpu()

            kl_penalty = [xs[start: ends[ix]] for ix, xs in enumerate(kl_penalty)]
            step_kl_penalty = [xs[start: ends[ix]] for ix, xs in enumerate(step_kl_penalty)]

            mean_kl = np.mean([(k / -self.kl_ctl.value).sum() for k in kl_penalty])
            mean_step_kl = np.mean([(k / -self.kl_ctl.value).sum() for k in step_kl_penalty])
            logging2.warning(f"mean kl {mean_kl}")
            logging2.warning(f"mean step kl {mean_step_kl}")
            ...
            stats["policy/step_wise_kl"] = mean_step_kl
        ...
        stats = {k: sum([xs[k] for xs in accumulated_stats]) / len(accumulated_stats) for k in
                 stats}
    ...
    def save_pretrained(self, directory: Optional[str] = None, **kwargs):
        ...
        state_dict = self.accelerator.get_state_dict(self.model.base_model, unwrap=True)
