...
class AccelerateRLTrainer(BaseRLTrainer):
        ...
        for prompt, sample, prompt_size in zip(prompts, samples, prompt_sizes):
            ...
            if append_eos_token and (
                ...
                if sample[-1] == self.tokenizer.pad_token_id:
                    str_output += self.tokenizer.pad_token
                else:
                    str_output += self.tokenizer.eos_token
    ...
    def evaluate(self):  # noqa: C901
        ...
        if self.kl_fn is not None:
            stats["KL"] = []
        if self.kl_forward_fn is not None:
            stats["KL_forward"] = []
        ...
        for i_sweep, gen_sweep_value in enumerate(gen_sweep_values):
            ...
            for i_prompt, prompts in enumerate(self.eval_dataloader):
                ...
                if self.kl_fn is not None:
                    sample_str = self.tokenizer.batch_decode(samples, skip_special_tokens=True)
                    kl_batch = self.kl_fn(sample_str, self.model)
                    # logging2.warning(f"KL: {kl_batch}")
                    stats["KL"].extend(kl_batch)
                
                if self.kl_forward_fn is not None:
                    prompts_str = self.tokenizer.batch_decode(prompts, skip_special_tokens=True)
                    kl_forward_batch = self.kl_forward_fn(prompts_str, self.model)
                    logging2.warning(f"KL: {kl_forward_batch}")
                    stats["KL_forward"].extend(kl_forward_batch)
            ...
            if self.kl_fn is not None:
                stats["KL"] = torch.tensor(stats["KL"]).mean().item()
            
            if self.kl_forward_fn is not None:
                stats["KL_forward"] = torch.tensor(stats["KL_forward"]).mean().item()
